...
```js
let [biscuitArrayResponse, biscuitArrayError] = initSDK.generateBiscuits(
    biscuitPrivateKey,
    ["ETHEREUM.CONTRACTS", "ETHEREUM.TRANSACTIONS"],
    false,
...
```js
// Create a Schema
let [createSchemaResponse, createSchemaError] = await initSDK.CreateSchema(
    "CREATE SCHEMA ETH"
);
...
// for DROP, use DDL()
let [CreateTableResponse, CreateTableError] = await initSDK.CreateTable(
    "CREATE TABLE ETH.TESTTABLE(id INT PRIMARY KEY, test VARCHAR)",
    "permissioned",
    publickey,
    biscuitArray
);
...
// For DROP
let [DDLresponse, DDLerror] = await initSDK.DDL(
    "DROP TABLE ETH.TESTTABLE",
    biscuitArray
);
...
let [DMLResponse, DMLError] = await initSDK.DML(
    ["ETH.TESTTABLE"],
    "INSERT INTO ETH.TESTTABLE VALUES(5, 'X5')",
    biscuitArray
);
...
let [DQLResponse, DQLError] = await initSDK.DQL(
    ["ETH.TESTTABLE"],
    "SELECT * FROM ETH.TESTTABLE",
    biscuitArray
);
...
let [getTableColumnResponse, getTableColumnError] =
    await initSDK.getTableColumns("ETH", "TESTTABLE");
...
let [getTableIndexesResponse, getTableIndexesError] =
    await initSDK.getTableIndexes("ETH", "TESTTABLE");
...
let [getPrimaryKeyResponse, getPrimaryKeyError] = await initSDK.getPrimaryKeys(
    "ETH",
    "TESTTABLE"
);
...
let [tableRelationshipResponse, tableRelationshipError] =
    await initSDK.getTableRelationships("ETH", "PRIVATE");
...
let [primaryKeyReferenceResponse, primaryKeyReferenceError] =
    await initSDK.getPrimaryKeyReferences("ETH", "TESTTABLE", "TEST");
...
let [foreignKeyReferenceResponse, foreignKeyReferenceError] =
    await initSDK.getForeignKeyReferences("ETH", "TESTTABLE", "TEST");
...
let [getBlockchainResponse, getBlockchainError] =
    await initSDK.getBlockchains();

console.log("Response: ", getBlockchainResponse);
console.log("Error: ", getBlockchainError);

// Get schemas for the provided blockchain
let [getBlockchainSchemasResponse, getBlockchainSchemasError] =
    await initSDK.getBlockchainSchemas("ETHEREUM");
...
let [getBlockchainInformationResponse, getBlockchainInformationError] =
    await initSDK.getBlockchainInformation("ETHEREUM");
...
```js
// File
SpaceAndTimeInit.write_to_file(
    AccessToken,
    RefreshToken,
    AccessTokenExpires,
    RefreshTokenExpires
