 ...
 */
const CheckExistingSession = async (sessionFilePath, sxt) => {
    ...
    const credentials = authorization.GenerateKeyPairFromString(credentialsJson);
    keypair = credentials;
    return true;
};
/**
 ...
 * SQL Core APIs
 */
const SQLAPIUtil = async (sxt, keypair) => {
    // Create Biscuits
    ...
    const authorization = sxt.Authorization();
    const biscuit = await authorization.CreateBiscuitToken(requiredBiscuit, keypair.biscuitPrivateKeyHex_32);
    ...
    // READ
    const readData = await sqlAPI.DQL(`SELECT * FROM ${resourceName}`, biscuitArray, resourceArray);
